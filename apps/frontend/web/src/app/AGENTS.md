...
| `<arena-side-nav-section>`                           | it cannot be prerendered. Its `ngAfterContentInit` guard spreads `host.nativeElement.children`, and the DOM `@angular/platform-server` ships returns a collection with no `Symbol.iterator`, so every panel route dies with "children is not iterable" and `bun run build` writes nothing. **`<arena-side-nav-collapsible>` is the heading that survives a prerender**: it reads no DOM, its children register by injection, and it opens itself around the active destination. `layout/panel-nav.ts:railEntries()` folds the rail into what it takes |
