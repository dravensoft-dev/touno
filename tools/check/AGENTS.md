...
| `check-docs.ts`       | a document passes `MAX_DOCUMENT_CHARS`, prose outside a fence or a code span carries an em dash, or a hand-written source under one of `SCANNED_TREES` carries a comment. `SIZE_ALLOWANCE` is empty, and **that emptiness is the claim**: a document holding a raised limit that has fallen back inside the shared one fails as a stale allowance, so the pressure to decompose it returns rather than ending                                     |
