...
from import shops on one network, one cart, one code and one tracking story. Angular 22 prerendered
...
[`src/app/domain/AGENTS.md`](./src/app/domain/AGENTS.md), and what the business promises the people
who use it is [`TOUNO_STRUC.md`](./TOUNO_STRUC.md).
...
| --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| a page throws at prerender, or renders differently after hydration, and I do not know which layer owns it | [`src/app/AGENTS.md`](./src/app/AGENTS.md), the prerender contract, then the level it names            |
| an Arena component does not take what I wrote, or takes it and draws nothing                              | the idiom table in [`src/app/AGENTS.md`](./src/app/AGENTS.md)                                          |
| something crosses the right edge of a phone                                                               | [`scripts/overflow-sweep/AGENTS.md`](./scripts/overflow-sweep/AGENTS.md)                               |
...
| an order, a recruitment, a fare, a reputation figure, a fixture                                           | [`src/app/domain/AGENTS.md`](./src/app/domain/AGENTS.md)                                               |
| the rail, the app bar, the gate, the theme, a notice                                                      | [`src/app/layout/AGENTS.md`](./src/app/layout/AGENTS.md)                                               |
| a routed page, in any of the six lanes                                                                    | [`src/app/pages/AGENTS.md`](./src/app/pages/AGENTS.md), then that lane's own                           |
...
| how anything is allowed to look: a colour, a step, a class, an icon                                       | [`design/AGENTS.md`](./design/AGENTS.md)                                                               |
| a value the skin answers with                                                                             | [`design/touno/AGENTS.md`](./design/touno/AGENTS.md), the style kernel                                 |
| a build-time script                                                                                       | [`scripts/AGENTS.md`](./scripts/AGENTS.md)                                                             |
| a gate, or a claim I want something to fail on                                                            | [`scripts/check/AGENTS.md`](./scripts/check/AGENTS.md)                                                 |
...
- `@angular/ssr` with `outputMode: "static"`. **Every route is prerendered and there is no server at
  runtime.** Nothing may depend on a request, a header, a cookie, or a runtime environment variable.
  A new dynamic route needs an entry in `src/app/app.routes.server.ts` with `getPrerenderParams`, or
  it does not exist in the output.

## Comments

- **Write no comments. Anywhere. Ever.** Not in TypeScript, not in HTML, not in CSS, not in JSON.
  `bun run check:docs` fails one.
...
- A gate states its reason in an exported reason string and in its row in
  [`scripts/check/AGENTS.md`](./scripts/check/AGENTS.md), which is the better record: a stale entry
...
[`src/app/seo/AGENTS.md`](./src/app/seo/AGENTS.md).

**`public/CNAME` is what binds the artefact to that host.** GitHub Pages reads it from the root of
...
[`src/app/layout/AGENTS.md`](./src/app/layout/AGENTS.md) for a rail destination and a breadcrumb, and
[`src/app/shared/AGENTS.md`](./src/app/shared/AGENTS.md) for a card's href and its cover.
...
bun run test           # Vitest over src/ and scripts/
...
- **No horizontal overflow from 320px up.** `bun run serve:static`, then `bun run sweep:overflow`.
  **A wrapped bar is not an overflow and no width measurement can see one**, so when you change what
  ...
  [`src/app/layout/AGENTS.md`](./src/app/layout/AGENTS.md) says why, and
  [`scripts/overflow-sweep/AGENTS.md`](./scripts/overflow-sweep/AGENTS.md) says what the sweep does
  see.
...
- **Walk the four order journeys end to end**, switching profiles as the parcel changes hands, and
  ...
  which screen is [`src/app/domain/AGENTS.md`](./src/app/domain/AGENTS.md), under
...
- **Read the Manual from every rail**, and confirm each lands on its own role and offers the way back
  ...
  [`src/app/pages/public/AGENTS.md`](./src/app/pages/public/AGENTS.md).
